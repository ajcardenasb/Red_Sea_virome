...
all.l1$Typevvir$Presumed[match(all.l1$Family, rownames(vir))]
...
#### Viral family average relative abundances

dna=read.table("./Input_files/MG_filtered_counts", header = TRUE)
dna.r=as.data.frame(t(sweep(dna,2,colSums(dna),"/")))
dna.r$Species=gsub("[0-9]", "", rownames(dna.r))
dna.agg=aggregate(dna.r[, 1:117], list(dna.r$Species), mean)

Parvo=dna.agg %>% select(Group.1, Parvoviridae)
Hepad=dna.agg %>% select(Group.1, Hepadnaviridae)
Dici=dna.agg %>% select(Group.1, Dicistroviridae)

rna=read.table("./Input_files/MT_filtered_counts", header = TRUE)
rna.r=as.data.frame(t(sweep(rna,2,colSums(rna),"/")))
rna.r$Species=gsub("[0-9]", "", rownames(rna.r))
rna.agg=aggregate(rna.r[, 1:117], list(rna.r$Species), mean)
Pico=rna.agg %>% select(Group.1, Picobirnaviridae)
Siph=rna.agg %>% select(Group.1, Siphoviridae)
